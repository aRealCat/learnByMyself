...
export function showWarningModal (content, title = '警告') {
  return new Promise((resolve) => {
    MessageBox.alert(content, title, {comfirmButtonText: '确定', confirmButtonClass: 'el-button--danger', lockScroll: true, type: 'error'}).then(() => {
      resolve()
    })
  })
}
...
export function http ({url, data = {}, method = 'post', noErrorDialog = false}) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    if (method === 'post') {
      if (store.state.config[currentRoute.name] && store.state.config[currentRoute.name].menuId) {
        data.menuId = store.state.config[currentRoute.name].menuId
      }
      httpParms.data = data
    }
    axios(httpParms).then((res) => {
      loading.close()
      if (Array.isArray(res.data) || typeof res.data === 'object') { // !Array.isArray(res.data) && typeof res.data === 'object'
